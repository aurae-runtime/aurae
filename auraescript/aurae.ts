...
export function createClient(opts: CreateClient = { kind: "default" }): Promise<number> {
    ...
    switch (opts.kind) {
        case "default": {
            config = Deno.core.opAsync("as__aurae_config__try_default");
            break;
        }
        case "path": {
            config = Deno.core.opAsync("as__aurae_config__parse_from_file", opts.path);
            break;
        }
        case "opts": {
            config = Deno.core.opAsync("as__aurae_config__from_options",
    ...
    // @ts-ignore
    return Deno.core.opAsync("as__client_new", config);
}
